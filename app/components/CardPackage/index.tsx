...
import { formatNumber } from '@/app/dashboard/admin/ads/constant/converter';
...
const PackageItem = ({ selectedPackage, setSelectedPackage, tourTarget, firstBuyer, prices }: any) => {
    ...
    if (!packages.length) return <ProgressSpinner style={{ width: '50px', height: '50px' }} strokeWidth="8" fill="var(--surface-ground)" animationDuration=".5s" />;
    return (
        <div className="surface-0">
            <div className="card flex justify-content-center package-card-selector " dir="ltr">
                <SelectButton style={{minWidth:'50%'}} value={selectedPackage} onChange={(e: SelectButtonChangeEvent) => setSelectedPackage(e.value)} optionLabel="name" options={items} multiple={false} />
            </div>
        </div>
    );
};
