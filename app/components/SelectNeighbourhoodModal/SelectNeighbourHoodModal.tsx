...
export default function SelectNeighbourHoodModal({ isVisible, header, maxSelection, onOk, onCancel, okText, cancelText, closable = false }: SelectNeighbourHoodModalProps) {
    ...
    return (
        <Dialog header={header ? header : <h2> انتخاب محله‌ها</h2>} onHide={onCancel} closable={closable} visible={isVisible}>
            <div className={styles.dialog_wrapper}>
                <LoadingPage isLoading={loading}>
                    <div className={styles.chipContainer}>
                        {selectedNeighbourhoods.map((item, index) => {
                            return <Chip className={classNames(styles.chip, 'gap-2 align-items-center')} label={item.name} removable key={item.id} onRemove={() => onRemoveItem(index)} />;
                        })}
                    </div>
