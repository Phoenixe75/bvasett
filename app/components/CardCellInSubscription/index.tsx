...
import { getSubscriptionAds } from '@/app/dashboard/user/my-subscriptions/(services)/mySubscriptions.service';
...
export default function CardCell({id} : CardCellProps) {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            let decodedResults = {}
            if (results) {
                decodedResults = JSON.parse(decodeURIComponent(results));
            }
            const response = await getSubscriptionAds(id,decodedResults, page);
            ...
            if(isAxiosError(error) && error.status === 403) {
                setErrorText('این اشتراک در بازه ی زمانی فعال نمیباشد')
            }
        } finally {
            setLoading(false);
    ...
    return (
        ...
        <div className="col-12 p-0 lg:pb-5 mt-4 lg:mt-0 surface-card grid">
            <div className="field col-12 flex justify-content-center">
                <h5>
            {ads?.length === 0 && errorText}

                </h5>
            </div>
            {ads?.map((item,idx) => {
                return <div key={idx} className="field col-12 md:col-4"><ItemCard showSelectable={false} onShowDetails={showDetails} data={item} key={item.id} isSelected={false} onClick={()=>{}} selectable={false}/></div>
            })}
             {next ? (
