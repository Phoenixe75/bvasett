...
import { orderBuy, paymentRequestService, paymentService } from './(services)/orderBuy.service';
import { IOrderBuyRequest, IOrderBuyResponse, IPaymentStatus, ItemBasedOrderBuyRequest } from './(models)/orderBuy';
        ...
        <>
            ...
            <div className="card p-fluid">
                ...
                <div className='flex align-items-center justify-content-between w-full'>
                    {hasShownInquryModal ? <h5 className='m-2'>لیست آگهی‌های جایگزین</h5> : <h5 className='m-2'>لیست آگهی‌های یافت شده</h5>}
                    {hasShownInquryModal ? <h5 className='m-2'>{`تعداد آگهی جایگزین انتخابی:${extraSelectedRows.length}`}</h5> : null}
                ...
                <div className="filess">
                    <ItemCards key={hasShownInquryModal.toString()} data={cardsData} selectable={!!selectedPackage} selectedPackage={selectedPackage || undefined} onSelectionChange={handleSelectionChange} maxCount={maxCount} onShowDetails={showDetails} />
                </div>
                {next ? (
                ...
                <div className="field flex col-12 md:col-2 gap-2">
                    <Button raised severity='success' type="submit" label="خرید" onClick={handleBuyClick} disabled={!selectedPackage} loading={btnLoading} />
                    ...
                    <form onSubmit={submitForm}>
                        ...
                        <div >
                            ...
                            <div className='flex gap-3 align-items-center'>
                                ...
                                {extraSelectedRows.length ? <p className="mr-2">تعداد فایل جایگزین: {selectedRows?.length} عدد </p> : null}
                                {orderInfo?.discount ? <p className="mr-2">{` تخفیف اعمال شده: ${formatMoneyToPersianUnit(Number(orderInfo?.discount),{returnZero:true})}`} </p>: null}
                                {orderInfo?.total ? <p className="mr-2">{`هزینه‌ی کل: ${formatMoneyToPersianUnit(Number(orderInfo?.total),{returnZero:true})}`}</p> : null}
                                <p/>
                            </div>
                        </div>
                        <hr />
                ...
                <Dialog header="خرید با استعلام" visible={showInquiryModal} onHide={() => setShowInquiryModal(false)} className="lg:w-6 md:w-50">
                    <hr />
                    <div>
                        <h5>راهنمای خرید با استعلام</h5>
                        <p className="mr-2"> به دلیل نیاز به استعلام و احتمال عدم فعال بودن آگهی انتخابی شما، نیاز است به تعداد انتخابی خود پکیج جایگزین انتخاب کنید.</p>
                    </div>
                    <hr />
                    <div className="field flex col-12 md:col-12 justify-content-end mt-4 ">
                        <Button raised onClick={onChoseExtraClick} label="انتخاب موارد جایگزین" className="bg-green-500 text-white border-0 mt-2 ml-2" disabled={btnLoading} loading={btnLoading} />
                        <Button raised label="انصراف" type="button" className="bg-gray-300 text-color border-0 mt-2" onClick={() => setShowInquiryModal(false)} />
                    </div>
                </Dialog>
            </div>
