...
// import NewTikcet from '../create/page';
...
const QuestionsList = () => {
    ...
    useEffect(() => {
        const fetchUserNames = async () => {
            if (data.length > 0) {
                const userIds = data.map((question) => question.user);
                try {
    ...
    const getItems = (rowData: IQuestions): MenuItem[] => {
        return [
            {
                label: 'مشاهده',
                icon: 'pi pi-eye text-yellow-500 ml-2',
                command: () => router.push(`users-questions/details/${rowData.id}`)
            },
            {
                label: 'پاسخ دادن',
                icon: 'pi pi-replay text-primary ml-2',
                command: () => router.push(`users-questions/replay/${rowData.id}`)
            },
            {
                label: 'ویرایش',
                icon: 'pi pi-pencil text-primary ml-2',
                command: () => router.push(`users-questions/edit/${rowData.id}`)
            }
            // {
            //     label: 'حذف',
            //     icon: 'pi pi-trash text-red-500 ml-2',
            //     command: () => {
            //         setSelectedQuestionId(rowData.id!);
    ...
    const refreshData = async () => {
        try {
            setLoading(true);
            const data = await getAllQuestions(page + 1);
    ...
    const confirmDelete = async () => {
        if (selectedQuestionId != null) {
            try {
                //await onDeleteQuestion(selectedQuestionId);
    ...
    const detailsBodyTemplate = (rowData: IQuestions, options: ColumnBodyOptions) => {
        return (
            <React.Fragment>
        ...
        <>
            <div>
                ...
                <DataTable value={data} paginator={false} rows={rows} tableStyle={{ minWidth: '50rem' }} className="p-datatable-gridlines">
                    ...
                    <Column field="answered_at" header="پاسخ  توسط" body={(rowData) => truncateText(rowData.answered_at, 12)}></Column>
                    <Column field="answered_by" header="پاسخ  توسط 2" body={(rowData) => truncateText(rowData.answered_by, 12)}></Column>
                    <Column
                        field="created"
                    ...
                    <Column field="user" header=" کاربر" body={(rowData) => userNames[data.findIndex((question) => question.id === rowData.id)] || 'نامشخص'}></Column>
