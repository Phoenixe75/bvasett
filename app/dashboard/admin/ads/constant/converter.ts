...
export const getPurposeLabelwithColor = (purpose: PurposeEnum | null): { label: string; className: string; iconClass: string, bgColor:string, color:string } => {
    switch (purpose) {
        case PurposeEnum.Buy:
            return { label: 'فروش', className: 'bg-yellow-200', bgColor: '#FF0000', color:'white', iconClass: 'text-yellow-700' };
        case PurposeEnum.Rent:
...
export const getPrice = (data:any) => {
    switch (data.purpose) {
        ...
        case PurposeEnum.Rent:
            return {total:formatMoneyToPersianUnit(data.rent_pre_paid_amount), rent:formatMoneyToPersianUnit(data.rent_price)};
        case PurposeEnum.Barter:
...
function toPersianNumber(str:string):string {
    ...
    //@ts-ignore
    return str.toString().replace(/[0-9]/g, (digit) => enToFaDigits[digit]);
  }
