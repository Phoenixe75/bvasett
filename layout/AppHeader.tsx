...
const AppHeader = ({toggleMenuItemClick, isHidden, loadingIsFinished, setIsHidden}: any) => {
    ...
    return (
      <>
        ...
        <div className='flex gap-1'>
          {!loading && !user?.is_staff && !user?.is_admin && user?.mobile ?
            <Button raised label="ثبت آگهی"
                    text
                    rounded
                    className="border-1 border-red-500 font-light line-height-2 text-gray-100 customBtn"
                    onClick={() => router.push('/dashboard/user/submit-ad')}/> : null}
  ...
  return (
    ...
    <div
      ...
      <StyleClass nodeRef={menuRef as NodeRef} selector="@next" enterClassName="hidden" leaveToClassName="hidden"
                  hideOnOutsideClick>
        <i ref={menuRef} className="pi pi-bars text-4xl cursor-pointer block lg:hidden text-700 miniMenu"></i>
      </StyleClass>
      {/* , backgroundColor: '#083b19' */}
      <div
        className={classNames('align-items-center flex-grow-1 justify-content-between hidden lg:flex absolute lg:static w-full left-0 px-6 lg:px-0 z-2 mr-3 navcolor bg-header', {hidden: isHidden})}
        style={{top: '100%'}}>
        <ul className="list-none p-0 m-0 flex lg:align-items-center select-none flex-column lg:flex-row cursor-pointer">
          <li>
            <Link href="/" onClick={toggleMenuItemClick}
                  className="p-ripple flex m-0 md:ml-5 px-0 py-3 text-900 hover:text-cyan-700 font-medium line-height-3 align-items-center gap-2">
          ...
          <li>
            <Link href="/articles" onClick={toggleMenuItemClick}
                  className="p-ripple flex m-0 md:ml-5 px-0 py-3 text-900 hover:text-cyan-700 font-medium line-height-3 align-items-center gap-2">
          ...
          <li>
            <Link href="/about" onClick={toggleMenuItemClick}
                  className="p-ripple flex m-0 md:ml-5 px-0 py-3 text-900 hover:text-cyan-700 font-medium line-height-3 align-items-center gap-2">
          ...
          <li>
            <Link href="/contact" onClick={toggleMenuItemClick}
                  className="p-ripple flex m-0 md:ml-5 px-0 py-3 text-900 hover:text-cyan-700 font-medium line-height-3 align-items-center gap-2">
        ...
        <div
          className="flex justify-content-between border-top-1 lg:border-top-none surface-border py-3 lg:py-0 mt-3 lg:mt-0">
          {loading &&
            <ProgressSpinner style={{width: '50px', height: '50px'}} strokeWidth="8" fill="var(--surface-ground)"
                             animationDuration=".5s"/>}
          {
            !loading && !user ? <Button raised label="ثبت آگهی"
                                        text
                                        rounded
                                        className="border-1 border-red-500 font-light line-height-2 text-gray-100 customBtn ml-2"
                                        onClick={mustLoginFirstHandler}/> : null
          }
          {!loading && !user && (
            <Button raised label="ورود / ثبت نام" icon="pi-sign-in pl-2 pt-1 text-lg" text rounded
                    className="border-1 border-red-500 font-light line-height-2 text-gray-100 customBtn  login-btn"
